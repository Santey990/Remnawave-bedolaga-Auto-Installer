...
# Модификация: порт Caddy принудительно 9443
# ============================================
...
# Глобальный алиас (опционально)
create_alias() {
    if [ ! -f /usr/local/bin/rw-installer ]; then
        echo -e "${YELLOW}📥 Создаём глобальный алиас...${NC}"
        cp "$0" /usr/local/bin/rw-installer
        chmod +x /usr/local/bin/rw-installer
        echo -e "${GREEN}✅ Теперь можно запускать командой rw-installer${NC}"
    fi
}
...
init_caddy_blocks() { mkdir -p /opt/remnawave/caddy/blocks; }

add_caddy_block() {
    local domain="$1"
    local block_content="$2"
    init_caddy_blocks
    echo "$block_content" > "/opt/remnawave/caddy/blocks/$domain"
...
install_panel() {
    ...
    echo -e "${YELLOW}🔐 Генерируем секретные ключи...${NC}"
...
EOC
    ...
    if [ -n "$API_TOKEN" ]; then
        echo -e "\n${YELLOW}📄 Устанавливаем страницу подписки...${NC}"
...
EOS
    ...
    echo -e "${CYAN}🌐 Админка: https://$PANEL_DOMAIN:$CADDY_HTTPS_PORT${NC}"
    echo -e "${CYAN}📄 Подписка: https://$SUB_DOMAIN:$CADDY_HTTPS_PORT${NC}"
    read -p "Нажмите Enter..."
}
...
# ---------- НАСТРОЙКА CADDY ДЛЯ BEDOLAGA (если нужно вручную) ----------
setup_caddy_for_bedolaga() {
    ...
    # Определяем домены автоматически (если есть)
    BOT_DOMAIN=""
    CABINET_DOMAIN=""
...
# ---------- ОСТАЛЬНЫЕ ФУНКЦИИ (Admin Bot, WARP, DDNS, Backup, Logs, Удаление) ----------
install_admin_bot() {
    show_logo
...
show_remnawave_menu() {
    while true; do
        ...
        echo -e "  ${CYAN}2)${NC} 🖥️  Установить Ноду (отдельный сервер)"
...
while true; do
    ...
    echo -e "  ${CYAN}1)${NC} 🚀 Remnawave (Панель, Нода)"
    echo -e "  ${CYAN}2)${NC} 💰 Bedolaga (Bot + Cabinet)"
